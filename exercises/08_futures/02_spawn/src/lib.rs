...
pub async fn echoes(first: TcpListener, second: TcpListener) -> Result<(), anyhow::Error> {
    // todo!()
    loop {
        let (mut socket_1, _) = first.accept().await?;
        ...
        tokio::spawn(async move {
            ...
            tokio::io::copy(&mut reader_1, &mut writer_1).await;
        });

        let (mut socket_2, _) = second.accept().await?;
        // Spawn a background task to handle the connection
        // thus allowing the main task to immediately start 
        // accepting new connections
        tokio::spawn(async move {
            let (mut reader_2, mut writer_2) = socket_2.split();
            tokio::io::copy(&mut reader_2, &mut writer_2).await;
        });
    }
}
