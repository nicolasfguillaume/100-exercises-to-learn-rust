...
// The iterator should yield immutable references to the tickets, ordered by their `TicketId`.
impl<'a> IntoIterator for &'a TicketStore {
    // The type of items being iterated over (here, a reference to each ticket), which live as long as the lifetime 'a.
    type Item = &'a Ticket;
    
    // The iterator type is Values<'a, TicketId, Ticket>.
    // This iterator yields references to the Tickets in the BTreeMap, which are already in order according to the TicketId keys.
    // The Values iterator doesn't include the keys, it only yields immutable references to the values (i.e., &Ticket).
    ...
    fn into_iter(self) -> Self::IntoIter {
        // BTreeMap already maintains sorted order, so we can directly return an iterator over values
        self.tickets.values()
    }
}
