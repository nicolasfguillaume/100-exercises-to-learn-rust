...
impl TryFrom<String> for TicketDescription {
    ...
    fn try_from(s: String) -> Result<Self, Self::Error> {
        match s {
            description if description.is_empty() => Err(TicketDescriptionError::DescriptionCannotBeEmpty),
            description if description.len() > 500 => Err(TicketDescriptionError::DescriptionTooLong),
            description => Ok(TicketDescription(description)),
        }
    }
}
